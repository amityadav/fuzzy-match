<?php
	...
	if($db){
		if(mysql_select_db($database)){
			...
			while ($rowSite = mysql_fetch_assoc($resultSet)) {
				//$srmSiteRecords[$rowSite['id']] = trim(addslashes($rowSite['address1'] . ' ' . $rowSite['address2'] . ' ' . $rowSite['city'] . ' ' . $rowSite['state'] . ' ' . $rowSite['country'] . ' ' . $rowSite['zipcode']));
				$siteStr = '';
			...
			while ($rowStudent = mysql_fetch_assoc($resultSet)) {
					...
					//$srmStudentRec = trim(addslashes($rowStudent['address1'] . ' ' . $rowStudent['address2'] . ' ' . $rowStudent['city'] . ' ' . $rowStudent['state'] . ' ' . $rowStudent['country'] . ' ' . $rowStudent['zipcode']));
					$srmStudentRec = '';
					...
					if(is_numeric($keyMatched)){
						...
						mysql_query("INSERT INTO srm_sites SET srm_company_id=0, division=" . trim(addslashes($rowStudent['division'])) . ", department=" . trim(addslashes($rowStudent['department'])) . ",address1=" . trim(addslashes($rowStudent['address1'])) . ", address2=" . trim(addslashes($rowStudent['address2'])) . ", city=" . trim(addslashes($rowStudent['city'])) . ", state=" . trim(addslashes($rowStudent['state'])) . ", country=" . trim(addslashes($rowStudent['country'])) . ", zipcode=" . trim(addslashes($rowStudent['zipcode'])));
	...
	function get_matching_address($srmStudentRec, $srmSiteRecords, $srmStudentRecId){
		...
		if($keyMatched){ // A MATCH HAS BEEN FOUND FOR THE STUDENT ADDRESS
